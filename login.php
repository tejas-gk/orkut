...
if(!isset($_SESSION['is_login'])){
  if(isset($_REQUEST['rEmail'])){
    $rEmail = mysqli_real_escape_string($mysqli,trim($_REQUEST['rEmail']));
    $rPassword = mysqli_real_escape_string($mysqli,trim($_REQUEST['rPassword']));
    $sql = "SELECT pemail, ppassword FROM profile_info WHERE pemail='".$rEmail."' AND ppassword='".$rPassword."' limit 1";
